...
public class CellPolicyFactory extends BOAbstractFactory implements IBOCellPolicy {
   protected final GuiCtx gc;

   public CellPolicyFactory(GuiCtx gc) {
      super(gc.getBOC());
      this.gc = gc;
   }
    ...
    * <br>
    * <br>
    * @param type {@link IBOCellPolicy#OFFSET_01_TYPE1}
    * @param numCells {@link IBOCellPolicy#OFFSET_05_CELL_NUM2}
    * @param numVisible {@link IBOCellPolicy#OFFSET_06_NUM_VISIBLE1}
    * @param policy {@link IBOCellPolicy#OFFSET_07_POLICY1}
    * @param sizeCell {@link IBOCellPolicy#OFFSET_09_SIZE4}
    * @param sizeMin {@link IBOCellPolicy#OFFSET_10_SIZE_MIN2}
    * @param sizeMax {@link IBOCellPolicy#OFFSET_11_SIZE_MAX2}
    * @return
    */
   private ByteObject get(int type, int numCells, int numVisible, int policy, int sizeCell, int sizeMin, int sizeMax) {
      ByteObject p = getBOFactory().createByteObject(IBOTypesGui.TYPE_122_CELL_POLICY, CELLP_BASIC_SIZE);
      p.setValue(OFFSET_01_TYPE1, type, 1);
      p.setValue(OFFSET_05_CELL_NUM2, numCells, 2);
      p.setValue(OFFSET_06_NUM_VISIBLE1, numVisible, 1);
      p.setValue(OFFSET_07_POLICY1, policy, 1);
      p.setValue(OFFSET_09_SIZE4, sizeCell, 4);
      p.setValue(OFFSET_10_SIZE_MIN2, sizeMin, 2);
      p.setValue(OFFSET_11_SIZE_MAX2, sizeMax, 2);
      return p;
   }
   ...
   public ByteObject getFlow(int sizeCell, boolean isStrong) {
      ByteObject p = getBOFactory().createByteObject(IBOTypesGui.TYPE_122_CELL_POLICY, CELLP_BASIC_SIZE);
      p.setValue(OFFSET_01_TYPE1, TYPE_1_FLOW, 1);
      p.setValue(OFFSET_09_SIZE4, sizeCell, 4);
      p.setFlag(OFFSET_02_FLAG, FLAG_5_STRONG_FLOW, isStrong);
      return p;
   }
   ...
   public ByteObject getGeneric(int numCells, int size) {
      ByteObject p = getBOFactory().createByteObject(IBOTypesGui.TYPE_122_CELL_POLICY, CELLP_BASIC_SIZE);
      p.setValue(OFFSET_01_TYPE1, TYPE_0_GENERIC, 1);
      p.setValue(OFFSET_05_CELL_NUM2, numCells, 2);
      if (size == 0) {
         p.setValue(OFFSET_07_POLICY1, CELL_0_IMPLICIT_SET, 1);
      } else {
         p.setValue(OFFSET_07_POLICY1, CELL_1_EXPLICIT_SET, 1);
      }
      p.setValue(OFFSET_09_SIZE4, size, 4);
      return p;
   }
   ...
   public ByteObject getGenericEtalon(int numCells, int size, int etalon) {
      ...
      p.setValue(OFFSET_07_POLICY1, CELL_1_EXPLICIT_SET, 1);
      p.setValue(OFFSET_09_SIZE4, size, 4);
      p.setValue(OFFSET_08_ETALON1, etalon, 1);
      p.setFlag(OFFSET_02_FLAG, FLAG_2_ETALON, true);
      return p;
   }
   ...
   public ByteObject getPreferedMaxView(int numCells) {
      ...
      int etalonSub = ITechLayoutDrawable.ETALON_TYPE_VIEWPORT;
      int etalonFunction = ITechLayout.ET_FUN_0_CTX;
      int etalonProp = ITechSizer.SIZER_PROP_05_CONTENT;
   ...
   public ByteObject getPrefered(int numCells, ByteObject maxSizer) {
      ByteObject p = getBOFactory().createByteObject(IBOTypesGui.TYPE_122_CELL_POLICY, CELLP_BASIC_SIZE);
      p.set1(OFFSET_01_TYPE1, TYPE_0_GENERIC);
      p.set2(OFFSET_05_CELL_NUM2, numCells);
      p.set1(OFFSET_07_POLICY1, CELL_0_IMPLICIT_SET);
      if (maxSizer != null) {
         p.addSub(maxSizer);
      }
      return p;
    ...
    * @param sizes
    * @return
    * @see CellPolicyFactory#getGeneric(int, int[], int[], int[], int[])
    */
   ...
   public ByteObject getGeneric(int numCells, int[] policies, int[] sizes, int[] minSize, int[] maxSize, int[] frames) {
      ...
      ByteObject cellPolicy = gc.getBOC().getByteObjectFactory().createByteObject(IBOTypesGui.TYPE_122_CELL_POLICY, totalSize);

      cellPolicy.setValue(OFFSET_01_TYPE1, TYPE_0_GENERIC, 1);
      cellPolicy.setValue(OFFSET_05_CELL_NUM2, numCells, 2);

      if (max1 > numCells || max2 > numCells) {
         cellPolicy.setFlag(OFFSET_02_FLAG, FLAG_6_CYCLICAL, true);
      }

      //sets the array flag 
      cellPolicy.setValue(OFFSET_04_FLAGZ, holders[INDEX_FLAGS], 1);

      //write the arrays
      if (holders[INDEX_ARRAY_SIZE] == 1) {
         cellPolicy.setValue(OFFSET_07_POLICY1, policies[0], 1);
      } else {
         cellPolicy.setDynOverWriteValues(offsets[0], policies, holders[0]);
      }
      if (holders[INDEX_ARRAY_SIZE + 1] == 1) {
         cellPolicy.setValue(OFFSET_09_SIZE4, sizes[0], 4);
      } else {
         cellPolicy.setDynOverWriteValues(offsets[1], sizes, holders[1]);
      }
      if (holders[INDEX_ARRAY_SIZE + 2] == 1) {
         cellPolicy.setValue(OFFSET_10_SIZE_MIN2, minSize[0], 2);
      } else {
         cellPolicy.setDynOverWriteValues(offsets[2], minSize, holders[2]);
      }
      if (holders[INDEX_ARRAY_SIZE + 3] == 1) {
         cellPolicy.setValue(OFFSET_11_SIZE_MAX2, maxSize[0], 2);
      } else {
         cellPolicy.setDynOverWriteValues(offsets[3], maxSize, holders[3]);
      }
      if (holders[INDEX_ARRAY_SIZE + 4] == 1) {
         cellPolicy.setValue(OFFSET_12_FRAMES1, frames[0], 1);
      } else {
    ...
    * <br>
    * <br>
    * There is cyclical phasse {@link IBOCellPolicy#FLAG_6_CYCLICAL} when policy or size array is 2 or more
    * and the number of cells does not match.
    * <br>
    * For example, 0, {@link IBOCellPolicy#CELL_1_EXPLICIT_SET} and Size[20,40].
    * <br>
    * 
   ...
   public ByteObject getGeneric(int numCells, int[] policies, int[] sizes, int[] minSize, int[] maxSize) {
      ...
      ByteObject p = gc.getBOC().getByteObjectFactory().createByteObject(IBOTypesGui.TYPE_122_CELL_POLICY, totalSize);

      p.setValue(OFFSET_01_TYPE1, TYPE_0_GENERIC, 1);
      p.setValue(OFFSET_05_CELL_NUM2, numCells, 2);

      if (max1 > numCells || max2 > numCells) {
         p.setFlag(OFFSET_02_FLAG, FLAG_6_CYCLICAL, true);
      }

      //sets the array flag 
      p.setValue(OFFSET_04_FLAGZ, holders[INDEX_FLAGS], 1);

      //write the arrays
      if (holders[INDEX_ARRAY_SIZE] == 1) {
         p.setValue(OFFSET_07_POLICY1, policies[0], 1);
      } else {
         p.setDynOverWriteValues(offsets[0], policies, holders[0]);
      }
      if (holders[INDEX_ARRAY_SIZE + 1] == 1) {
         p.setValue(OFFSET_09_SIZE4, sizes[0], 4);
      } else {
         p.setDynOverWriteValues(offsets[1], sizes, holders[1]);
      }
      if (holders[INDEX_ARRAY_SIZE + 2] == 1) {
         p.setValue(OFFSET_10_SIZE_MIN2, minSize[0], 2);
      } else {
         p.setDynOverWriteValues(offsets[2], minSize, holders[2]);
      }
      if (holders[INDEX_ARRAY_SIZE + 3] == 1) {
         p.setValue(OFFSET_11_SIZE_MAX2, maxSize[0], 2);
      } else {
    ...
    * @param sizes
    * @return
    * @see CellPolicyFactory#getGeneric(int[], int[], int[], int[])
    */
   ...
   /**
    * 
    * {@link ByteObject} opaque with Even Ratio {@link IBOCellPolicy#FLAG_4_RATIO_EVEN}
    * <br>
    * <br>
    * @param numCells
    * @param numVisible
   ...
   public ByteObject getRatio(int numCells, int numVisible, int size) {
      ...
      p.setFlag(OFFSET_02_FLAG, FLAG_4_RATIO_EVEN, true);
      return p;
   }
